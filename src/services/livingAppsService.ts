...
import type { Mitarbeiterverwaltung, Schichtdefinitionen, Schichtplanung, CreateMitarbeiterverwaltung, CreateSchichtdefinitionen, CreateSchichtplanung } from '@/types/app';
...
export class LivingAppsService {
  ...
  // --- SCHICHTDEFINITIONEN ---
  static async getSchichtdefinitionen(): Promise<Schichtdefinitionen[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.SCHICHTDEFINITIONEN}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Schichtdefinitionen[];
    return enrichLookupFields(records, 'schichtdefinitionen');
  }
  static async getSchichtdefinitionenEntry(id: string): Promise<Schichtdefinitionen | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.SCHICHTDEFINITIONEN}/records/${id}`);
    const record = { record_id: data.id, ...data } as Schichtdefinitionen;
    return enrichLookupFields([record], 'schichtdefinitionen')[0];
  }
  static async createSchichtdefinitionenEntry(fields: CreateSchichtdefinitionen) {
    return callApi('POST', `/apps/${APP_IDS.SCHICHTDEFINITIONEN}/records`, { fields: cleanFieldsForApi(fields as any, 'schichtdefinitionen') });
  }
  static async updateSchichtdefinitionenEntry(id: string, fields: Partial<CreateSchichtdefinitionen>) {
    return callApi('PATCH', `/apps/${APP_IDS.SCHICHTDEFINITIONEN}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'schichtdefinitionen') });
  }
  static async deleteSchichtdefinitionenEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.SCHICHTDEFINITIONEN}/records/${id}`);
  }
