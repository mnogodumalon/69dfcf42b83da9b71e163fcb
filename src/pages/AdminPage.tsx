...
const SCHICHTPLANUNG_FIELDS = [
  ...
  { key: 'schicht', label: 'Schicht', type: 'applookup/select', targetEntity: 'schichtdefinitionen', targetAppId: 'SCHICHTDEFINITIONEN', displayField: 'schichtname' },
  { key: 'einsatzbereich', label: 'Einsatzbereich', type: 'string/text' },
...
export default function AdminPage() {
  ...
  const getLookupLists = useCallback((entity: EntityKey) => {
    ...
    switch (entity) {
      case 'schichtplanung':
        ...
        lists.schichtdefinitionenList = (data as any).schichtdefinitionen ?? [];
        break;
    }
    return lists;
  }, [data]);
  ...
  const getApplookupDisplay = useCallback((entity: EntityKey, fieldKey: string, url?: unknown) => {
    ...
    if (entity === 'schichtplanung' && fieldKey === 'mitarbeiter') {
      const match = (lists.mitarbeiterverwaltungList ?? []).find((r: any) => r.record_id === id);
      return match?.fields.vorname ?? '—';
    }
    if (entity === 'schichtplanung' && fieldKey === 'schicht') {
      const match = (lists.schichtdefinitionenList ?? []).find((r: any) => r.record_id === id);
