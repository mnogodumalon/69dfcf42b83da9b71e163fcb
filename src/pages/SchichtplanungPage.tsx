...
export default function SchichtplanungPage() {
  ...
  const [schichtdefinitionenList, setSchichtdefinitionenList] = useState<Schichtdefinitionen[]>([]);
  ...
  async function loadData() {
    setLoading(true);
    try {
      const [mainData, mitarbeiterverwaltungData, schichtdefinitionenData] = await Promise.all([
      ...
      setSchichtdefinitionenList(schichtdefinitionenData);
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    ...
    >
        ...
        <Table className="[&_tbody_td]:px-6 [&_tbody_td]:py-2 [&_tbody_td]:text-base [&_tbody_td]:font-medium [&_tbody_tr:first-child_td]:pt-6 [&_tbody_tr:last-child_td]:pb-10">
          <TableHeader className="bg-secondary">
            <TableRow className="border-b border-input">
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('einsatzbereich')}>
                <span className="inline-flex items-center gap-1">
                  Einsatzbereich
                  {sortKey === 'einsatzbereich' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('vertretung')}>
                <span className="inline-flex items-center gap-1">
                  Vertretung
                  {sortKey === 'vertretung' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
          ...
          <TableBody>
            {sortRecords(filtered).map(record => (
              <TableRow key={record.record_id} className="hover:bg-muted/50 transition-colors cursor-pointer" onClick={(e) => { if ((e.target as HTMLElement).closest('button, [role="checkbox"]')) return; setViewingRecord(record); }}>
                ...
                <TableCell><span className="inline-flex items-center bg-secondary border border-[#bfdbfe] text-[#2563eb] rounded-[10px] px-2 py-1 text-sm font-medium">{getSchichtdefinitionenDisplayName(record.fields.schicht)}</span></TableCell>
                <TableCell className="font-medium">{record.fields.einsatzbereich ?? '—'}</TableCell>
                <TableCell><span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium ${record.fields.vertretung ? 'bg-primary/10 text-primary' : 'bg-muted text-muted-foreground'}`}>{record.fields.vertretung ? 'Ja' : 'Nein'}</span></TableCell>
      ...
      <SchichtplanungDialog
        ...
        schichtdefinitionenList={schichtdefinitionenList}
